...
router.get("/", async (req, res) => {
  try {
    ...
    try {
      const [[{ db: currentDb }]] = await db.execute(
    ...
    // optional limit for rows returned
    const rawLimit = Number(req.query.limit || 200);
    const limit = Math.min(isNaN(rawLimit) ? 200 : rawLimit, 1000);

    // 1) fetch the recent history rows
    ...
    // total recruiters
    const [[recRow]] = await db.execute(
      `SELECT COUNT(*) AS total_recruiters FROM \`RECRUITER\``
    );

    // total freelancers
    const [[freeRow]] = await db.execute(
    ...
    return res
      .status(500)
      .json({ ok: false, error: "Failed to fetch job history" });
  }
});
